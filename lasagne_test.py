...
from data_utils import load_data, export_csv
...
def build_mlp(input_var=None):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, 1, 144, 128),
                                     input_var=input_var)
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.InputLayer(shape=(None, 1, 144, 128),
                                        input_var=input_var)
...
def main(model='mlp', num_epochs=500):
    ...
    y_pred = np.argmax(predict_function(X_test), axis=1)

    export_csv(y_pred,'kaggle.csv')
